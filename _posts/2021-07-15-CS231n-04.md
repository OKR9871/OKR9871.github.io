...
지금까지는 scalar값에 대한 back propagation에 대해서 공부했습니다. 지금부터는 vector에 대한 back propagtion에 대해서 배우는데 기존에 배워왔던거와 같습니다.    
...
추가적으로 ${\partial q_{k}\over \partial W_{i,j}}={1_{k=i}x_{j}}$식이 있는데 나는 가장 이해하기 힘든 표기 법이였지만 다시 생각해보니 간단한 문제였다.   
${1_{k=i}x_{j}}$ 를 보면 k가 i일때 1을 $x_{j}$와 곱해주겠다는 의미인데 여기서 $i$는 행, $j$는 열 이다. 이는 같은 행이 아닌 다른 행은 연산시 사용하지 않겠다는 의미 입니다.    

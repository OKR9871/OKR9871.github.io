...
따라서 분류문제에 적합한 logistic regression에 대해서 설명하겠습니다.    
...
Logistic regression의 cost functino은 다음과 같이 표기할 수 있습니다.    
...
아래 그림과 같이 underfit은 학습데이터에 너무 맞지 않아 bias가 큰경우를 말하고, overfit은 학습데이터에는 맞지만 variance가 큰 경우를 말합니다.    
...
따라서 overfitting을 막기위해서 2가지의 선택지가 있는데 그중 하나는 feature의 개수를 줄이는 방법, 다른 하나는 parameter $\theta$의 값을 줄이는 방법이 있습니다.    
 ...
 이러한 식을 통해 $\theta$의 값을 작게 유지하도록 만듭니다.    
  ...
  $\lambda$가 너무 크게 되면 underfitting되고 말고, $\lambda$가 너무 작게 되면 규제의 효과를 얻지 못하게 됩니다.   

...
$J_{\theta}=-{1\over m}[\sum \limits_{i=1}^m \sum\limits_{k=1}^K {y_{k}^{(i)}log(h_{\theta}(x^{(i)}))_{k}}+(1-y_{k}^{(i)})log(1-(h_{\theta}(x^{(i)}))_{k})]+{\lambda \over 2m}{\sum\limits_{l=1}^{L-1}\sum\limits_{i=1}^{s_{l}}\sum\limits_{j=1}^{s_{l}+1}(\theta_{ji}^{(l)})^2}$   
